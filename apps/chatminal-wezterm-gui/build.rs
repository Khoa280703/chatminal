...
fn main() {
    ...
    println!(
        ...
        repo_root
            .join("scripts")
            .join("bootstrap-wezterm-vendor-deps.sh")
            .display()
    );
...
fn ensure_vendored_native_deps(repo_root: &Path) {
    ...
    if !status.success() {
        panic!(
            "bootstrap wezterm vendor deps failed with status {status}: {}",
            script.display()
        );
    }
}
...
BEGIN
    ...
    BEGIN
        ...
        BEGIN
            ...
            VALUE "FileDescription",  "Chatminal WezTerm GUI\0"
            VALUE "FileVersion",      "{version}\0"
            VALUE "InternalName",     "chatminal-wezterm-gui\0"
            VALUE "OriginalFilename", "chatminal-wezterm-gui.exe\0"
