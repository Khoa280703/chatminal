...
impl super::TermWindow {
    fn is_secondary_click_event(event: &MouseEvent) -> bool {
        ...
        ) || event.mouse_buttons.contains(WMB::RIGHT)
            || (matches!(
                event.kind,
    ...
    fn resolve_sidebar_resize_item(&self, event: &MouseEvent) -> Option<UIItem> {
        if !self.chatminal_sidebar.is_enabled() {
            return None;
        }

        let sb = self.shell_bounds();
        if !sb.shell_enabled || sb.sidebar_width <= 0.0 {
            return None;
        }

        const RESIZE_HANDLE_HALF_WIDTH_PX: isize = 4;
    ...
    fn dismiss_chatminal_sidebar_context_menu(&mut self, context: &dyn WindowOps) {
        if self.chatminal_sidebar.close_session_context_menu() {
            context.invalidate();
        }
    }
    ...
    fn resolve_ui_item(&self, event: &MouseEvent) -> Option<UIItem> {
        if let Some(item) = self.resolve_sidebar_resize_item(event) {
            return Some(item);
        }
    ...
    pub fn mouse_event_impl(&mut self, event: MouseEvent, context: &dyn WindowOps) {
        ...
        if let Some(modal) = self.get_modal() {
            let modal_mouse_event = terminal_emulator::MouseEvent {
        ...
        let ui_item = if matches!(self.current_mouse_capture, None | Some(MouseCapture::UI)) {
            ...
            match (self.last_ui_item.take(), &ui_item) {
                (Some(prior), Some(item)) => {
                    if prior != *item || !self.config.use_fancy_session_bar {
    ...
    fn drag_split(
        ...
        if delta != 0 {
            if let Some(split) = self.resize_split_via_tab_capability(split, delta) {
                item.item_type = UIItemType::Split(split);
                context.invalidate();
            } else if self.chatminal_sidebar.is_enabled() {
                if let Some(split) = self.resize_workspace_layout_split(split, delta) {
                    item.item_type = UIItemType::Split(split);
                    context.invalidate();
                }
            } else {
    ...
    fn drag_chatminal_sidebar_resize_handle(
        &mut self,
        item: UIItem,
        start_event: MouseEvent,
        event: MouseEvent,
        ...
        let requested_width_px = event.coords.x.max(0) as f32;
        if self.chatminal_sidebar.set_width_pixels(
            requested_width_px,
    ...
    fn mouse_event_ui_item(
        ...
        match item.item_type {
            ...
            UIItemType::CloseSessionEntry(idx) => {
                self.mouse_event_close_session_entry(idx, event, context);
            }
            UIItemType::CloseSession(session_id) => {
                self.mouse_event_close_session(&session_id, event, context);
            }
            UIItemType::ChatminalSidebarBackground => {
                self.mouse_event_chatminal_sidebar_background(event, context);
            }
            UIItemType::ChatminalSidebarResizeHandle => {
                self.mouse_event_chatminal_sidebar_resize_handle(item, event, context);
            }
            UIItemType::ChatminalSidebarSettings => {
                self.mouse_event_chatminal_sidebar_settings(event, context);
            }
            UIItemType::ChatminalSidebarCreateProfile => {
    ...
    fn mouse_event_chatminal_sidebar_background(
        &mut self,
        event: MouseEvent,
        context: &dyn WindowOps,
    ) {
    ...
    fn mouse_event_chatminal_sidebar_create_profile(
        &mut self,
        event: MouseEvent,
        context: &dyn WindowOps,
    ) {
    ...
    fn mouse_event_chatminal_sidebar_resize_handle(
        &mut self,
        item: UIItem,
        event: MouseEvent,
        context: &dyn WindowOps,
    ...
    fn mouse_event_chatminal_sidebar_settings(
        &mut self,
        event: MouseEvent,
        context: &dyn WindowOps,
    ) {
    ...
    fn mouse_event_chatminal_sidebar_profile(
        &mut self,
        profile_id: &str,
        event: MouseEvent,
        context: &dyn WindowOps,
        ...
        if Self::is_secondary_click_event(&event) {
            if matches!(
                ...
                let menu_changed = self.chatminal_sidebar.open_profile_context_menu(
                    profile_id,
    ...
    fn mouse_event_chatminal_sidebar_create_session(
        &mut self,
        event: MouseEvent,
        context: &dyn WindowOps,
    ) {
    ...
    fn mouse_event_chatminal_sidebar_profile_menu(
        &mut self,
        event: MouseEvent,
        context: &dyn WindowOps,
    ) {
    ...
    fn mouse_event_chatminal_sidebar_session_menu_rename(
        &mut self,
        session_id: &str,
        event: MouseEvent,
        context: &dyn WindowOps,
    ...
    fn mouse_event_chatminal_sidebar_session_menu_delete(
        &mut self,
        session_id: &str,
        event: MouseEvent,
        context: &dyn WindowOps,
    ...
    fn mouse_event_chatminal_sidebar_profile_menu_delete(
        &mut self,
        profile_id: &str,
        event: MouseEvent,
        context: &dyn WindowOps,
    ...
    pub fn mouse_event_close_session_entry(
        &mut self,
        idx: usize,
        event: MouseEvent,
        context: &dyn WindowOps,
        ...
        match event.kind {
            WMEK::Press(MousePress::Left) => {
                ...
                self.close_runtime_entry_at_index(idx, true);
            }
            _ => {}
        }
        context.set_cursor(Some(MouseCursor::Arrow));
    ...
    pub fn mouse_event_session_bar(
        &mut self,
        item: SessionBarItem,
        event: MouseEvent,
        context: &dyn WindowOps,
        ...
        match event.kind {
            WMEK::Press(MousePress::Left) => match item {
                SessionBarItem::RuntimeEntry { entry_idx, .. } => {
                    ...
                    self.activate_runtime_entry_index(entry_idx as isize).ok();
                }
                ...
                SessionBarItem::None | SessionBarItem::LeftStatus | SessionBarItem::RightStatus => {
                    let maximized = self
                        .window_state
                ...
                SessionBarItem::WindowButton(button) => {
            ...
            WMEK::Press(MousePress::Middle) => match item {
                SessionBarItem::RuntimeEntry { entry_idx, .. } => {
                    ...
                    self.close_runtime_entry_at_index(entry_idx, true);
                }
                SessionBarItem::Session { session_id, .. } => {
                    self.close_chatminal_session_by_id(&session_id);
                }
                SessionBarItem::NewSessionButton => {
                ...
                SessionBarItem::None
                | SessionBarItem::LeftStatus
                | SessionBarItem::RightStatus
            ...
            WMEK::Press(MousePress::Right) => match item {
                SessionBarItem::RuntimeEntry { .. } => {
                    ...
                    self.show_runtime_entry_navigator();
                }
                SessionBarItem::Session { .. } => {}
                SessionBarItem::NewSessionButton => {
            ...
            WMEK::Move => match item {
                ...
                SessionBarItem::WindowButton(window::IntegratedTitleButton::Maximize) => {
            ...
            WMEK::VertWheel(n) => {
                if self.config.mouse_wheel_scrolls_tabs {
                    ...
                    self.activate_runtime_entry_relative(delta, true).ok();
                }
            }
            _ => {}
        }
    ...
    pub fn mouse_event_split(
        ...
        context.set_cursor(Some(match &split.direction {
            TerminalSplitDirection::Horizontal => MouseCursor::SizeLeftRight,
            TerminalSplitDirection::Vertical => MouseCursor::SizeUpDown,
        }));
    ...
    fn mouse_event_terminal(
        ...
        let mouse_event = terminal_emulator::MouseEvent {
            kind: match event.kind {
                WMEK::Move => TMEK::Move,
                WMEK::VertWheel(_) | WMEK::HorzWheel(_) | WMEK::Press(_) => TMEK::Press,
                WMEK::Release(_) => TMEK::Release,
