...
use terminal_font::units::{IntPixelLength, PixelLength};
use terminal_font::{ClearShapeCache, GlyphInfo, LoadedFont};
use terminal_emulator::color::{ColorAttribute, ColorPalette};
use terminal_emulator::{CellAttributes, Line, StableRowIndex};
use euclid::num::Zero;
...
#[path = "termwindow/render/borders.rs"]
pub mod borders;
#[path = "termwindow/render/chatminal_sidebar.rs"]
pub mod chatminal_sidebar;
#[path = "termwindow/render/corners.rs"]
pub mod corners;
#[path = "termwindow/render/draw.rs"]
pub mod draw;
#[path = "termwindow/render/fancy_tab_bar.rs"]
pub mod fancy_tab_bar;
#[path = "termwindow/render/paint.rs"]
pub mod paint;
#[path = "termwindow/render/pane.rs"]
pub mod pane;
#[path = "termwindow/render/screen_line.rs"]
pub mod screen_line;
#[path = "termwindow/render/split.rs"]
pub mod split;
#[path = "termwindow/render/tab_bar.rs"]
pub mod tab_bar;
#[path = "termwindow/render/window_buttons.rs"]
...
pub struct RenderScreenLineParams<'a> {
    ...
    pub config: &'a ConfigHandle,
    pub pane: Option<&'a Arc<dyn OverlayPane>>,
...
fn resolve_fg_color_attr(
    ...
    match fg {
        terminal_emulator::color::ColorAttribute::Default => {
        ...
        terminal_emulator::color::ColorAttribute::PaletteIndex(idx)
            if idx < 8 && config.bold_brightens_ansi_colors != BoldBrightening::No =>
        {
            // For compatibility purposes, switch to a brighter version
            // of one of the standard ANSI colors when Bold is enabled.
            // This lifts black to dark grey.
            let idx = if attrs.intensity() == terminal_emulator::Intensity::Bold {
                idx + 8
            } else {
                idx
            };

            palette.resolve_fg(terminal_emulator::color::ColorAttribute::PaletteIndex(idx))
        }
