...
pub fn restore_persisted_layout(workspace_id: &str) -> Option<WorkspaceLayoutState> {
    match crate::runtime_module::workspace_layout_restore_persisted(workspace_id) {
        Ok(layout) => layout,
        Err(err) => {
...
fn persist_layout_for_test(layout: &WorkspaceLayoutState) {
    if let Err(err) = crate::runtime_module::store_runtime_layout(layout) {
...
fn clear_persisted_layout_for_test() {
    if let Err(err) = crate::runtime_module::clear_runtime_layout_state() {
        log::error!("failed to clear persisted workspace layout: {err}");
    }
}

pub fn clear_layout(workspace_id: &str) {
    if let Err(err) = crate::runtime_module::workspace_layout_remove(workspace_id) {
...
impl DesktopWorkspaceLayoutStore {
    ...
    pub fn snapshot(&self) -> Option<WorkspaceLayoutState> {
        ...
        match crate::runtime_module::workspace_layout_snapshot(&self.workspace_id) {
            Ok(layout) => layout,
            Err(err) => {
    ...
    pub fn split_view(
        ...
        match crate::runtime_module::workspace_layout_split_view(
            &self.workspace_id,
            view_id,
            axis,
            session_id,
    ...
    pub fn focus_view(&self, view_id: SessionViewId) -> Option<WorkspaceLayoutState> {
        ...
        match crate::runtime_module::workspace_layout_focus_view(&self.workspace_id, view_id) {
            Ok(layout) => layout,
            Err(err) => {
    ...
    pub fn close_view(&self, view_id: SessionViewId) -> Option<WorkspaceLayoutState> {
        ...
        match crate::runtime_module::workspace_layout_close_view(&self.workspace_id, view_id) {
            Ok(layout) => layout,
            Err(err) => {
    ...
    pub fn resize_split(
        ...
        match crate::runtime_module::workspace_layout_resize_split(
            &self.workspace_id,
            node_id,
            ratio,
        ) {
    ...
    pub fn view_id_for_session(&self, session_id: &str) -> Option<SessionViewId> {
        ...
        match crate::runtime_module::workspace_layout_view_id_for_session(
            &self.workspace_id,
            session_id,
        ) {
            Ok(view_id) => view_id,
    ...
    pub fn replace_layout(&self, layout: WorkspaceLayoutState) -> WorkspaceLayoutState {
        ...
        match crate::runtime_module::workspace_layout_replace(&self.workspace_id, layout) {
            Ok(layout) => layout,
            Err(err) => {
