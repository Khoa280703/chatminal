...
pub(crate) mod overlay_shell {
    use runtime::SplitDirection;

    pub use runtime::pane::{
    ...
    pub type OverlayRuntimeEntryHandle = runtime::RuntimeId;
...
pub(crate) use runtime::pane::Pane as HostTerminal;
pub(crate) const ROOT_HOST_WINDOW_ID: PrimaryHostWindowId = window::ROOT_WINDOW_ID;

pub(crate) use runtime::pane::alloc_pane_id as alloc_host_terminal_handle;
pub(crate) use runtime::pane::impl_get_logical_lines_via_get_lines as host_impl_get_logical_lines_via_get_lines;
pub(crate) use runtime::renderable::{
...
pub(crate) fn install_lua_bridge_backend_once() {
    let _ = lua_bridge::install_backend(Arc::new(DesktopLuaBridgeBackend));
}
