...
use engine_dynamic::{ToDynamic, Value};
...
impl InputMap {
    ...
    pub fn new(config: &ConfigHandle) -> Self {
        ...
        if !config.disable_default_key_bindings {
            for (mods, code, action) in CommandDef::default_key_assignments(config) {
                ...
                // normalization under X11: upstream issue #1906.
    ...
    pub fn dump_config(&self, key_table: Option<&str>) {
        println!("local chatminal = require 'chatminal'");
        println!("local act = chatminal.action");
        println!();
        println!("return {{");
