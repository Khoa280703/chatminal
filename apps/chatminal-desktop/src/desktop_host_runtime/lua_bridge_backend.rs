...
use super::{
    ...
    resolve_host_runtime_id_for_terminal_handle, rotate_host_runtime_entry_clockwise_by_session_id,
    rotate_host_runtime_entry_counter_clockwise_by_session_id, session_id_from_pane_metadata,
    ...
    set_host_runtime_entry_title_by_session_id, set_host_runtime_entry_zoomed_by_session_id,
...
impl LuaBridgeBackend for DesktopLuaBridgeBackend {
    ...
    fn set_session_zoomed(&self, session_id: &str, zoomed: bool) -> anyhow::Result<bool> {
        set_host_runtime_entry_zoomed_by_session_id(session_id, zoomed)
            .ok_or_else(|| anyhow::anyhow!("session '{session_id}' not found in runtime"))
    }
    ...
    fn rotate_session_counter_clockwise(&self, session_id: &str) -> anyhow::Result<()> {
        rotate_host_runtime_entry_counter_clockwise_by_session_id(session_id)
            .then_some(())
            .ok_or_else(|| anyhow::anyhow!("session '{session_id}' not found in runtime"))
    }
    fn rotate_session_clockwise(&self, session_id: &str) -> anyhow::Result<()> {
        rotate_host_runtime_entry_clockwise_by_session_id(session_id)
            .then_some(())
            .ok_or_else(|| anyhow::anyhow!("session '{session_id}' not found in runtime"))
    }
