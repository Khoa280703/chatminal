...
use super::{
    alloc_host_terminal_handle, clamp_preview_lines, host_impl_get_logical_lines_via_get_lines,
    ...
    host_terminal_with_lines_mut, EmbeddedRuntime, HostCachePolicy as CachePolicy,
...
use crate::chatminal_runtime::{
    activate_runtime_session, load_workspace_passive, read_session_snapshot, ChatminalRuntimeClient,
};
...
pub(crate) struct ChatminalRuntimePane {
...
impl ChatminalRuntimePane {
    pub(crate) fn new(
        ...
        let pane = Arc::new(Self {
            pane_id: alloc_host_terminal_handle(),
            domain_id,
    ...
    fn activate_and_seed(&self, size: TerminalSize) -> anyhow::Result<()> {
        let session_status = load_workspace_passive()
...
impl HostTerminal for ChatminalRuntimePane {
