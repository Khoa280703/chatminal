...
use super::{EngineRuntimeAdapter, RuntimeId, SessionRuntimeState, TerminalInstanceId};
...
impl SessionFocusManager {
    ...
    pub fn focus_direction<A: EngineRuntimeAdapter>(
        ...
        let Some(target_terminal_instance_id) =
            adapter.adjacent_active_terminal_instance(runtime_id, direction)?
        else {
            return Ok(None);
        };
...
mod tests {
    ...
    use super::super::{
        EngineRuntimeAdapter, EngineRuntimeRef, LayoutNodeId, RuntimeId, SessionLayoutSnapshot,
        SessionRuntimeState, TerminalInstanceId,
    };
    ...
    fn focus_direction_uses_adapter_target_leaf() {
        ...
        assert_eq!(
            state.snapshot.active_terminal_instance_id,
            Some(super::TerminalInstanceId::new(2))
        );
        assert_eq!(
