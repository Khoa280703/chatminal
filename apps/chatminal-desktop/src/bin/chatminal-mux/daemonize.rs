...
fn fork() -> anyhow::Result<Fork> {
    ...
    if pid == 0 {
        Ok(Fork::Child(unsafe { libc::getpid() }))
    } else if pid < 0 {
        Err(std::io::Error::last_os_error()).context("fork")
    } else {
        Ok(Fork::Parent(pid))
    }
}
...
fn setsid() -> anyhow::Result<()> {
    ...
    if pid == -1 {
        Err(std::io::Error::last_os_error()).context("setsid")
    } else {
        Ok(())
    }
}
...
fn lock_pid_file(config: &config::ConfigHandle) -> anyhow::Result<std::fs::File> {
    ...
    let pid_dir = pid_file
        .parent()
        .ok_or_else(|| anyhow::anyhow!("{} has no parent?", pid_file.display()))?;
    std::fs::create_dir_all(pid_dir)
        .with_context(|| format!("while creating directory structure: {}", pid_dir.display()))?;
