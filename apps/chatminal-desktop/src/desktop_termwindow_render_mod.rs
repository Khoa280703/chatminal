...
use engine_font::units::{IntPixelLength, PixelLength};
use engine_font::{ClearShapeCache, GlyphInfo, LoadedFont};
use engine_term::color::{ColorAttribute, ColorPalette};
use engine_term::{CellAttributes, Line, StableRowIndex};
use euclid::num::Zero;
...
fn resolve_fg_color_attr(
    ...
    match fg {
        engine_term::color::ColorAttribute::Default => {
        ...
        engine_term::color::ColorAttribute::PaletteIndex(idx)
            if idx < 8 && config.bold_brightens_ansi_colors != BoldBrightening::No =>
        {
            // For compatibility purposes, switch to a brighter version
            // of one of the standard ANSI colors when Bold is enabled.
            // This lifts black to dark grey.
            let idx = if attrs.intensity() == engine_term::Intensity::Bold {
                idx + 8
            } else {
                idx
            };

            palette.resolve_fg(engine_term::color::ColorAttribute::PaletteIndex(idx))
        }
