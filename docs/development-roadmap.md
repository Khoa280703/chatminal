...
## Hoan Tat
1. Hoàn tất architecture unification Phase 01 (2026-04-01):
   - xóa dead config surface SSH/TLS/WSL
   - xóa auto-update config stubs
   - bỏ `libssh-rs` khỏi workspace
   - unify desktop shell startup path để `chatminal-desktop` không còn rơi về terminal-only mặc định
2. Tách daemon/client/protocol/store thành các crate/app độc lập.
3. Chuyển source tree sang runtime native-only theo hướng WezTerm core.
4. Dọn compatibility code legacy (`config.toml`, env typo aliases, text ping format).
5. Hoàn tất phase-01 native window foundation (`window-wezterm` + reducer tests + smoke).
6. Hoàn tất phase-02 daemon concurrency/perf:
...
9. Hoàn tất phase-05 quality gates/release dry-run:
   - fidelity/soak/release scripts có JSON report
   - checksum portable Linux/macOS
   - full gate suite pass liên tiếp
10. Hoàn tất batch hardening phase-05 follow-up (2026-03-05):
...
13. Hardening soak anti-flake (2026-03-05):
...
14. Đồng bộ phase checklist (2026-03-05):
...
15. Rollback guard mở rộng cho window backend (2026-03-05):
...
16. Đóng plan `260305-1458` (2026-03-05):
...
19. macOS native window input recovery (2026-03-06):
...
20. Restore vendored WezTerm GUI as default window runtime (2026-03-06):
...
21. Adopt Chatminal-owned WezTerm GUI package (2026-03-06):
...
22. Continue first-party WezTerm extraction (2026-03-06):
...
23. Continue helper/API extraction wave (2026-03-06):
...
24. Continue lua-api extraction around `env-bootstrap` (2026-03-06):
...
25. Continue first-party extraction wave around bootstrap/version/codec (2026-03-06):
...
26. Continue extraction of lightweight foundation crates (2026-03-06):
...
27. Continue extraction of parser/layout foundation crates (2026-03-06):
...
28. Continue extraction of color/char/input foundation crates (2026-03-06):
   ...
   - `cargo check -p chatminal-wezterm-gui` compile đi qua batch mới và vẫn chỉ dừng ở blocker native host Linux `xcb-util.pc`
   - `apps/chatminal-app` và `apps/chatminald` tiếp tục PASS full tests
29. Continue extraction of terminal data-path crates (2026-03-06):
   ...
   - `cargo check -p chatminal-wezterm-gui` compile đi qua batch mới và vẫn chỉ dừng ở blocker native host Linux `xcb-util.pc`
30. Continue extraction of runtime utility crates (2026-03-06):
   ...
   - `apps/chatminal-app` và `apps/chatminald` tiếp tục PASS full tests
31. Continue extraction of PTY/surface runtime crates (2026-03-06):
...
32. Continue extraction of SSH transport crate (2026-03-06):
...
33. Continue extraction of terminal core crate (2026-03-06):
   ...
   - `apps/chatminal-app` và `apps/chatminald` tiếp tục PASS full tests
34. Continue extraction of config crate (2026-03-06):
   ...
   - `apps/chatminal-app` và `apps/chatminald` tiếp tục PASS full tests
35. Continue extraction of mux crate (2026-03-06):
   ...
   - `cargo check -p chatminal-wezterm-gui` vẫn chỉ dừng ở blocker native host Linux `xcb-util.pc`
   - `apps/chatminal-app` và `apps/chatminald` tiếp tục PASS full tests
36. Continue extraction of termwiz crate and workspace cleanup (2026-03-06):
