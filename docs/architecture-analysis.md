...
**Status (2026-03-17 after Phase 2):**
...
- ✅ **Phase 2.1**: 17 Runtime* types merged as type aliases to chatminal-protocol; api/protocol.rs (431 LOC) **deleted**; 5 Store→Protocol From impls moved to chatminal-store
- ✅ **Phase 2.2**: Doc comments added to chatminal-terminal-core and chatminal-engine-term
- ⏳ **Next (Phase 3)**: Split layout deduplication + 5-to-2 ID mapping consolidation
...
| Vấn đề | Nghiêm trọng | Status | Đề xuất |
|---|---|---|---|
| **Split layout song song** | 🔴 Cao | ⏳ Phase 3 | Loại bỏ `Tab` split tree, dùng `WorkspaceLayoutState` làm single source of truth |
| **Identity mapping 5 loại ID** | 🔴 Cao | ⏳ Phase 3 | Giảm xuống 2 ID: `session_id` (business) + `PaneId` (engine) |
| **Data types trùng 3 tầng** | 🟡 Trung bình | ✅ Phase 2.1 partial | Runtime layer eliminado; Store↔Protocol conversion moved to chatminal-store |
| **Ghost crate references** | 🟢 Thấp | ⏳ Cleanup | Dọn docs/comments reference `chatminal-session-runtime` |
| **Unused engine features** | 🟡 Trung bình | ✅ Phase 1 | SSH/tmux/remote crates deleted; engine/domain features disabled |
| **Duplicate terminal parser** | 🟡 Trung bình | ⏳ Phase 3 | Chọn 1: hoặc `vt100` hoặc `termwiz` — tuỳ target |
| **third_party reference** | 🟢 Thấp | ⏳ Cleanup | Xoá subtree hoặc chuyển sang git submodule |
