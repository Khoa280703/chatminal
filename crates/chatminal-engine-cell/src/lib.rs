...
use core::mem;
pub use engine_char_props::emoji::Presentation;
use engine_char_props::emoji_variation::WCWIDTH_TABLE;
use engine_char_props::widechar_width::WcWidth;
use engine_dynamic::{FromDynamic, ToDynamic};
pub use engine_escape_parser::osc::Hyperlink;
...
pub use engine_escape_parser::csi::{Blink, Intensity, Underline, VerticalAlign};
...
pub fn is_white_space_char(c: char) -> bool {
    engine_char_props::white_space::WHITE_SPACE.contains_u32(c as u32)
}
...
///
/// The approach that Chatminal wants to take here is to define a
...
mod test {
    ...
    // That decision is made over in engine-term when processing
