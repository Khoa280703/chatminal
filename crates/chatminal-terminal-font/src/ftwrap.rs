...
use config::{ConfigHandle, FreeTypeLoadFlags, FreeTypeLoadTarget};
pub use freetype::*;
...
pub fn compute_load_flags_from_config(
    ...
    let config = config.cloned().unwrap_or_else(ConfigHandle::default_config);
...
impl Face {
    ...
    pub fn variations(&self, config: Option<&ConfigHandle>) -> anyhow::Result<Vec<ParsedFont>> {
        ...
        unsafe {
            ...
            for i in 1..=num_styles {
                ...
                res.push(ParsedFont::from_face(&self, source, config)?);
            }
    ...
    fn compute_cap_height(&mut self) -> anyhow::Result<f64> {
        ...
        let (load_flags, render_mode) =
            compute_load_flags_from_config(None, None, None, None, None);
...
impl Library {
    pub fn new(config: Option<&ConfigHandle>) -> anyhow::Result<Library> {
