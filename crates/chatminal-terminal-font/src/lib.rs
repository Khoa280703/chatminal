...
use engine_term::{CellAttributes, Intensity};
...
impl LoadedFont {
    ...
    pub fn rasterize_glyph(
        ...
        if let Some(raster) = rasterizers.get(&fallback) {
            ...
            let rasterizer_config = font_config.as_ref().map(|c| c.config.borrow().clone());
            let raster_selection = rasterizer_config
                .as_ref()
                .map_or(FontRasterizerSelection::default(), |c| c.font_rasterizer);
            let handles = self.handles.borrow();
            let raster = new_rasterizer(
                raster_selection,
                &handles[fallback],
                self.pixel_geometry,
                rasterizer_config.as_ref(),
            )?;
    ...
    fn process(self) {
        ...
        if wanted.len() > 1 && self.config.sort_fallback_fonts_by_coverage {
            ...
            extra_handles.sort_by_cached_key(|p| {
                p.coverage_intersection(&wanted, Some(&self.config))
                    .map(|r| r.len())
                    .unwrap_or(0)
            });
        ...
        if !wanted.is_empty() {
            ...
            if show_warning {
                ...
                let url = "https://github.com/Khoa280703/chatminal#readme";
                log::warn!(
...
impl FontConfigInner {
    ...
    pub fn new(config: Option<ConfigHandle>, dpi: usize) -> anyhow::Result<Self> {
        let config = config.unwrap_or_else(ConfigHandle::default_config);
            ...
            if !attr.is_synthetic && !attr.is_fallback && !loaded.contains(attr) {
                ...
                let explanation = if is_primary {
                    ...
                    format!(
                        ...
                        Note that Chatminal will synthesize font_rules to select bold \
                ...
                config::show_error(&format!(
                    ...
                    may not render as intended{}. See the font configuration \
    ...
    pub fn match_style<'a>(
        ...
        let would_bright = match attrs.foreground() {
            engine_term::color::ColorAttribute::PaletteIndex(idx) if idx < 8 => {
