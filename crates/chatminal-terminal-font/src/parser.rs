...
use config::{ConfigHandle, FontAttributes, FontStyle, FreeTypeLoadFlags, FreeTypeLoadTarget};
...
impl ParsedFont {
    ...
    pub fn lua_fallback(handles: &[Self]) -> String {
        let mut code = "chatminal.font_with_fallback({\n".to_string();
    ...
    pub fn from_face(
        ...
        if has_svg {
            let ignore_svg_fonts = config
                .cloned()
                .unwrap_or_else(ConfigHandle::default_config)
                .ignore_svg_fonts;
            if ignore_svg_fonts {
...
pub(crate) fn load_built_in_fonts(
    ...
    for bundle in built_ins {
        for (data, name) in bundle.iter() {
            ...
            let mut parsed = ParsedFont::from_face(&face, locator, config)?;
...
pub fn best_matching_font(
    ...
    parse_and_collect_font_info(source, &mut font_info, origin, config)?;
...
pub(crate) fn parse_and_collect_font_info(
    ...
    fn load_one(
        ...
        let face = lib.face_from_locator(&locator)?;
        if let Ok(variations) = face.variations(config) {
            for parsed in variations {
                font_info.push(parsed);
            }
        } else {
            let parsed = ParsedFont::from_locator(&locator, config)?;
            font_info.push(parsed);
        }
        Ok(())
    }
    ...
    for index in 0..num_faces {
        if let Err(err) = load_one(&lib, &source, index, font_info, &origin, config) {
