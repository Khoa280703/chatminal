...
use anyhow::Context;
use config::{current_config_handle, FontAttributes, FontStyle, FontWeight};
...
impl FontLocator for FontConfigFontLocator {
    ...
    fn locate_fallback_for_codepoints(
        ...
        // In upstream issue #4310 we discover
        ...
        'next_codepoint: for &c in codepoints {
            if !fonts.is_empty() {
                ...
                for f in &fonts {
                    match f.coverage_intersection(&wanted_range, Some(&runtime_config)) {
                        Ok(r) if !r.is_empty() => {
