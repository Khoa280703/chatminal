...
fn handles_from_descriptor(descriptor: &CTFontDescriptor) -> Vec<ParsedFont> {
    let runtime_config = config::current_config_handle();
    let mut result = vec![];
    ...
    fn locate_fallback_for_codepoints(
        ...
        for &c in codepoints {
            ...
            for font in candidates {
                ...
                if let Ok(cov) = font.coverage_intersection(&wanted, Some(&runtime_config)) {
...
fn build_fallback_list_impl() -> anyhow::Result<Vec<ParsedFont>> {
    ...
    for font in &fonts {
        if let Err(err) = font.coverage_intersection(&empty, Some(&runtime_config)) {
