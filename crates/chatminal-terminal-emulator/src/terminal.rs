...
pub trait DeviceControlHandler: Send + Sync {
    fn handle_device_control(&mut self, _control: engine_escape_parser::DeviceControlMode);
}
...
impl Terminal {
    ...
    pub fn perform_actions(&mut self, actions: Vec<engine_escape_parser::Action>) {
        self.state.increment_seqno();
        {
            let mut performer = Performer::new(&mut self.state);
