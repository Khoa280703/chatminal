...
pub const DEFAULT_WINDOW_CLASS: &str = "org.chatminal.desktop";
...
pub struct StartCommand {
    ...
    /// If enabled, don't try to ask an existing Chatminal Desktop instance
    /// to start the command.  Instead, always start the GUI in this
    /// invocation of Chatminal so that you can wait for the command
    /// to complete by waiting for this Chatminal process to finish.
    ...
    /// The default is "org.chatminal.desktop".
    ...
    /// instance of Chatminal, including error, update and ssh
...
pub struct SshCommand {
    ...
    /// `chatminal ssh` is able to parse some (but not all!) options
    ...
    /// For example:
    ///
    /// `chatminal ssh -oIdentityFile=/secret/id_ed25519 some-host`
    #[arg(
        long = "ssh-option",
        short = 'o',
        name = "name=value",
    ...
    /// Instead of executing your shell, run PROG.
    /// For example: `chatminal ssh user@host -- bash -l` will spawn bash
