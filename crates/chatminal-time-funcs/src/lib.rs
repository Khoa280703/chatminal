...
const SCHEDULED_EVENTS: &str = "chatminal-scheduled-events";
...
pub fn register(lua: &Lua) -> anyhow::Result<()> {
    ...
    api_mod.set("sleep_ms", lua.create_async_function(sleep_ms)?)?;
    api_mod.set("strftime", lua.create_function(strftime)?)?;
    api_mod.set("strftime_utc", lua.create_function(strftime_utc)?)?;
    Ok(())
}
