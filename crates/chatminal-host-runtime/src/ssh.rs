...
pub fn ssh_connect_with_ui(
    ssh_config: engine_ssh::ConfigMap,
    ui: &mut ConnectionUI,
...
pub fn ssh_domain_to_ssh_config(ssh_dom: &SshDomain) -> anyhow::Result<ConfigMap> {
    ...
    ssh_config.insert(
        "chatminal_ssh_backend".to_string(),
        match ssh_dom
            .ssh_backend
    ...
    if let Some("true") = ssh_config.get("chatminal_ssh_verbose").map(|s| s.as_str()) {
...
impl RemoteSshDomain {
    ...
    fn build_command(
        ...
        env.insert("CHATMINAL_REMOTE_PANE".to_string(), pane_id.to_string());
...
impl Domain for RemoteSshDomain {
    async fn spawn_pane(
        ...
        let StartNewSessionResult { pty, child, writer } = if let Some(session) = session.take() {
                ...
                .context("request ssh pty")
            {
                ...
                Err(err) => {
                    if err
                        .root_cause()
                        .downcast_ref::<engine_ssh::DeadSession>()
                        .is_some()
                    {
        ...
        let terminal = engine_term::Terminal::new(
            size,
