...
impl UserData for DomainRef {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        methods.add_meta_method(mlua::MetaMethod::ToString, |_, this, _: ()| {
            Ok(format!("DomainRef(domain_id:{}, pid:{})", this.0, unsafe {
                libc::getpid()
            }))
        });
        ...
        methods.add_async_method(
            "attach",
            |_, this, window: Option<UserDataRef<WindowRef>>| async move {
                let mux = get_mux()?;
                let domain = this.resolve(&mux)?;
        ...
        methods.add_method("has_any_leaves", |_, this, _: ()| {
            let mux = get_mux()?;
            let domain = this.resolve(&mux)?;
            let have_leaves_in_domain = mux
                .iter_panes()
                .iter()
                .any(|p| p.domain_id() == domain.domain_id());
            Ok(have_leaves_in_domain)
        });
    }
}
