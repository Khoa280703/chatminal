...
use engine_cell::image::ImageData;
use engine_cell::UnicodeVersion;
use engine_escape_parser::csi::{
...
use engine_escape_parser::{OneBased, OperatingSystemCommand, CSI};
use engine_surface::{CursorShape, CursorVisibility, SequenceNo};
use log::debug;
...
lazy_static::lazy_static! {
    static ref DB: Database = {
        let data = include_bytes!("../../data/chatminal");
...
impl TerminalState {
    ...
    /// Here in Chatminal the terminalstate is isolated from other
    ...
    fn perform_csi_mode(&mut self, mode: Mode) {
        match mode {
            ...
            Mode::SetDecPrivateMode(DecPrivateMode::Code(
                ...
                // This is handled in Chatminal's mux
            }
            Mode::ResetDecPrivateMode(DecPrivateMode::Code(
                DecPrivateModeCode::SynchronizedOutput,
            )) => {
            ...
            Mode::QueryDecPrivateMode(DecPrivateMode::Code(
                ...
                // This is handled in Chatminal's mux; if we get here, then it isn't enabled,
    ...
    fn checksum_rectangle(&mut self, left: u32, top: u32, right: u32, bottom: u32) -> u16 {
        ...
        */
        ...
        // The concept of uninitialized cells in Chatminal is not the same as that on VT520 or that
        ...
        // See: upstream pull request #4565
        if checksum == 0 {
            32u16
        } else {
            checksum
