...
impl FromDynamic for IntegratedTitleButtonStyle {
    fn from_dynamic(
        value: &dynamic::Value,
        _options: dynamic::FromDynamicOptions,
        ...
        Self: Sized,
    {
        ...
        if let dynamic::Value::String(string) = value {
            let style = match string.as_str() {
                "Windows" => Self::Windows,
                "Gnome" => Self::Gnome,
                "MacOsNative" if cfg!(target_os = "macos") => Self::MacOsNative,
                _ => {
                    return Err(dynamic::Error::InvalidVariantForType {
            ...
            Err(dynamic::Error::InvalidVariantForType {
