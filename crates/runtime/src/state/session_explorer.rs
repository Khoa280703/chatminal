...
use super::explorer_utils::{
    explorer_state_from_store, normalize_relative_path, resolve_explorer_root_path,
    resolve_explorer_target,
};
use crate::api::{
    RuntimeSessionExplorerEntry, RuntimeSessionExplorerFileContent, RuntimeSessionExplorerState,
};
...
impl StateInner {
    pub(super) fn get_session_explorer_state(
        &self,
        session_id: &str,
    ) -> Result<RuntimeSessionExplorerState, String> {
        self.ensure_session_exists(session_id)?;
        let state = self.store.get_session_explorer_state(session_id)?;
        Ok(explorer_state_from_store(session_id, state))
    }
    ...
    pub(super) fn set_session_explorer_root(
        ...
        Ok(explorer_state_from_store(session_id, Some(saved)))
    }
    ...
    pub(super) fn list_session_explorer_entries(
    ...
    ) -> Result<Vec<RuntimeSessionExplorerEntry>, String> {
        self.ensure_session_exists(session_id)?;
        ...
        for item in read_dir {
            ...
            entries.push(RuntimeSessionExplorerEntry {
                name,
    ...
    pub(super) fn read_session_explorer_file(
    ...
    ) -> Result<RuntimeSessionExplorerFileContent, String> {
        self.ensure_session_exists(session_id)?;
        let Some(state) = self.store.get_session_explorer_state(session_id)? else {
            return Err("session explorer root is not set".to_string());
        };
        ...
        Ok(RuntimeSessionExplorerFileContent {
