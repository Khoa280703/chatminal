...
use store::StoredSessionExplorerState;

use crate::api::RuntimeSessionExplorerState;

pub(super) fn explorer_state_from_store(
    session_id: &str,
    state: Option<StoredSessionExplorerState>,
) -> RuntimeSessionExplorerState {
    if let Some(value) = state {
        return RuntimeSessionExplorerState {
