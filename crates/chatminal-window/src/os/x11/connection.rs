...
pub struct XConnection {
    ...
    pub(crate) ime: RefCell<ImeClient>,
...
impl ConnectionOps for XConnection {
    ...
    fn screens(&self) -> anyhow::Result<Screens> {
        ...
        //
        // related issue: upstream issue #5802
        let res = match self
...
impl XConnection {
    ...
    fn process_xcb_event(&self, event: &xcb::Event) -> anyhow::Result<()> {
        match event {
            ...
            // because upstream issue #1992 is a resize related
    ...
    pub(crate) fn create_new() -> anyhow::Result<Rc<XConnection>> {
        ...
        let ime = ImeClient::new(&conn, screen_num);
        ...
        if config::configuration().ime_preedit_rendering == config::ImePreeditRendering::Builtin {
            ...
            conn.clone()
                .ime
                .borrow_mut()
                .set_preedit_draw_cb(move |window_id, text| {
                    if let Some(window) = conn.window_by_id(window_id) {
                        let mut inner = window.lock().unwrap();
                        let status = DeadKeyStatus::Composing(text.to_owned());
