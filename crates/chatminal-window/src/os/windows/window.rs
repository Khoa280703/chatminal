...
use engine_color_types::LinearRgba;
use engine_font::FontConfiguration;
use engine_input_types::KeyboardLedStatus;
...
impl Window {
    fn create_window(
        ...
        let class = WNDCLASSW {
            ...
            cbWndExtra: 0,
            hInstance: h_inst,
            // FIXME: this resource is specific to the Chatminal build and this should
...
unsafe fn update_title_font(hwnd: HWND) {
    ...
    if let Some(lf) = get_title_log_font(hwnd, hdc) {
        *font = engine_font::locator::gdi::parse_log_font(&lf, hdc).ok();
    }
