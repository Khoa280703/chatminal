...
const LUA_API_MODULE_NAME: &str = "chatminal";
const LUA_REGISTRY_USER_CALLBACK_COUNT: &str = "chatminal-user-callback-count";
const LUA_WATCH_PATHS: &str = "chatminal-watch-paths";
const LUA_EVENT_PREFIX: &str = "chatminal-event-";
const IS_EVENT: &str = "chatminal-is-event-emission";
...
pub fn get_or_create_sub_module<'lua>(
    ...
    let api_mod = get_or_create_module(lua, LUA_API_MODULE_NAME)?;
    let sub = api_mod.get(name)?;
    match sub {
        Value::Nil => {
            let sub = lua.create_table()?;
            api_mod.set(name, sub.clone())?;
            Ok(sub)
        }
        Value::Table(sub) => Ok(sub),
        wat => anyhow::bail!(
            "cannot register module chatminal.{name} as it is already set to a value of type {}",
            wat.type_name()
        ),
    }
}
        ...
        DynValue::Object(obj) => {
            match obj.get_by_str(&key) {
                None => {
                    ...
                    engine_dynamic::Error::warn(message);
                }
                Some(_dvalue) => {
...
/// Chatminal specific config paths for lua modules, should
/// they choose to `require` additional code from their config.
///
/// A `chatminal` module is registered so that the script can
/// `require "chatminal"` and call into functions provided by
/// Chatminal. The module contains:
/// * `executable_dir` - the directory containing the Chatminal
///   executable.  This is potentially useful for portable
///   installs on Windows.
/// * `config_dir` - the directory containing the Chatminal
///   configuration.
/// * `log_error` - a function that logs to stderr (or the server
///   log file for daemonized Chatminal).
/// * `target_triple` - the rust compilation target triple.
/// * `version` - the version of the running Chatminal instance.
...
pub fn make_lua_context(config_file: &Path) -> anyhow::Result<Lua> {
    ...
    {
        let globals = lua.globals();
        // This table will be the `chatminal` module in the script
        let api_mod = get_or_create_module(&lua, LUA_API_MODULE_NAME)?;
        ...
        prefix_path(&mut path_array, &crate::HOME_DIR.join(".chatminal"));
        ...
        if let Ok(exe) = std::env::current_exe() {
            if let Some(path) = exe.parent() {
                api_mod
                    ...
                    .context("set chatminal.executable_dir")?;
                if cfg!(windows) {
                    // For a portable windows install, force in this path ahead
                    // of the rest
                    prefix_path(&mut path_array, &path.join("chatminal_modules"));
                }
            }
        }
...
package.searchers[2] = function(module)
  ...
  if name then
    package.loaded.chatminal.add_to_config_reload_watch_list(name)
  end
  return orig(module)
end
        "#,
        ...
        api_mod
            ...
            .context("set chatminal.config_file")?;
        api_mod
            .set(
                "config_dir",
                config_dir
            ...
            .context("set chatminal.config_dir")?;

        lua.set_named_registry_value(LUA_WATCH_PATHS, Vec::<String>::new())?;
        api_mod.set(
            "add_to_config_reload_watch_list",
            lua.create_function(add_to_config_reload_watch_list)?,
        )?;

        api_mod.set("target_triple", crate::engine_target_triple())?;
        api_mod.set("version", crate::engine_version())?;
        api_mod.set("home_dir", crate::HOME_DIR.to_str())?;
        api_mod.set(
            "running_under_wsl",
        ...
        api_mod.set("hostname", lua.create_function(hostname)?)?;
        api_mod.set("action", luahelper::enumctor::Enum::<KeyAssignment>::new())?;
        api_mod.set(
            "has_action",
        ...
        api_mod.set("on", lua.create_function(register_event)?)?;
        api_mod.set("emit", lua.create_async_function(emit_event)?)?;
        api_mod.set("shell_join_args", lua.create_function(shell_join_args)?)?;
        api_mod.set("shell_quote_arg", lua.create_function(shell_quote_arg)?)?;
        api_mod.set("shell_split", lua.create_function(shell_split)?)?;
        ...
        // those values since Chatminal was started
...
///
/// `chatminal.font("foo", {foreground="tomato"})`
/// yields:
...
///
/// `chatminal.font_with_fallback({"Operator Mono", "DengXian"})`
///
/// The second optional argument is a list of other TextStyle fields,
/// as described by the `chatminal.font` documentation.
...
/// This implements `chatminal.on`, whose goal is to register an event handler
/// callback.
...
///
/// ```lua
/// chatminal.on("event-name", function(arg1, arg2)
///   -- do something
///   return false -- if you want to prevent other handlers running
/// end);
///
/// chatminal.emit("event-name", "foo", "bar");
/// ```
pub fn register_event<'lua>(
    lua: &'lua Lua,
    (name, func): (String, mlua::Function),
) -> mlua::Result<()> {
    let decorated_name = format!("{}{}", LUA_EVENT_PREFIX, name);
...
pub fn add_to_config_reload_watch_list<'lua>(
    ...
    let mut watch_paths: Vec<String> = lua.named_registry_value(LUA_WATCH_PATHS)?;
    watch_paths.extend_from_slice(&args);
    lua.set_named_registry_value(LUA_WATCH_PATHS, watch_paths)?;
    Ok(())
}
...
mod test {
    ...
    fn can_register_and_emit_multiple_events() -> anyhow::Result<()> {
        ...
        smol::block_on(
            lua.load(
                r#"
local chatminal = require 'chatminal';

chatminal.on('foo', function (n)
...
assert(chatminal.emit('foo', 2) == false)

chatminal.on('bar', function (n, str)
...
assert(chatminal.emit('bar', 42, 'woot') == true)
"#,
            )
            .exec_async(),
        )?;
