...
pub struct SshDomain {
    ...
    /// The path to the Chatminal binary on the remote host
    pub remote_chatminal_path: Option<String>,
    /// Override the entire `chatminal cli proxy` invocation that would otherwise
    /// be computed from remote_chatminal_path and other information.
    ...
    /// that the remote host have Chatminal installed, and is equivalent
    /// to using `chatminal ssh` to connect.
    #[dynamic(default)]
...
impl SshDomain {
    pub fn default_domains() -> Vec<Self> {
        let mut config = engine_ssh::Config::new();
        ...
        for host in config.enumerate_hosts() {
            ...
            mux_ssh.push(Self {
                ...
                remote_address: host.to_string(),
                multiplexing: SshMultiplexing::Chatminal,
                local_echo_threshold_ms: default_local_echo_threshold_ms(),
                ..SshDomain::default()
            });
        }
