...
impl ConfigInner {
    ...
    fn overridden(&mut self, overrides: &engine_dynamic::Value) -> Result<ConfigHandle, Error> {
...
impl Configuration {
    ...
    fn overridden(&self, overrides: &engine_dynamic::Value) -> Result<ConfigHandle, Error> {
        let mut inner = self.inner.lock().unwrap();
