...
use engine_config_derive::ConfigMeta;
...
pub struct Config {
    ...
    /// For example, to have Chatminal always run `top` by default,
    /// you'd use this:
    ///
    /// ```toml
    /// default_prog = ["top"]
    ...
    ///
    /// and you can set them in Chatminal:
    ///
    /// ```toml
...
impl Default for Config {
    fn default() -> Self {
        ...
        Config::from_dynamic(
            &engine_dynamic::Value::Object(Default::default()),
            Default::default(),
        )
        .unwrap()
    }
...
impl Config {
    ...
    pub fn load_with_overrides(overrides: &engine_dynamic::Value) -> LoadedConfig {
        ...
        let mut paths = vec![PathPossibility::optional(HOME_DIR.join(".chatminal.lua"))];
        for dir in CONFIG_DIRS.iter() {
            paths.push(PathPossibility::optional(dir.join("chatminal.lua")))
        }
        ...
        if cfg!(windows) {
            ...
            if let Ok(exe_name) = std::env::current_exe() {
                if let Some(exe_dir) = exe_name.parent() {
                    paths.insert(0, PathPossibility::optional(exe_dir.join("chatminal.lua")));
                }
            }
        }
        if let Some(path) = std::env::var_os("CHATMINAL_CONFIG_FILE") {
            log::trace!("Note: CHATMINAL_CONFIG_FILE is set in the environment");
        ...
        // We didn't find (or were asked to skip) a chatminal.lua file, so
        // update the environment to make it simpler to understand this
        // state.
        std::env::remove_var("CHATMINAL_CONFIG_FILE");
        std::env::remove_var("CHATMINAL_CONFIG_DIR");
    ...
    pub fn try_default() -> anyhow::Result<LoadedConfig> {
        let (config, warnings) =
            engine_dynamic::Error::capture_warnings(|| -> anyhow::Result<Config> {
    ...
    fn try_load(
        ...
        overrides: &engine_dynamic::Value,
    ...
    pub(crate) fn apply_overrides_obj_to<'l>(
        ...
        match overrides {
            engine_dynamic::Value::Object(obj) => {
    ...
    pub fn compute_extra_defaults(&self, config_path: Option<&Path>) -> Self {
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(true),
            intensity: Some(engine_term::Intensity::Half),
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(false),
            intensity: Some(engine_term::Intensity::Bold),
            font: bold,
            ..Default::default()
        });
        ...
        cfg.font_rules.push(StyleRule {
            italic: Some(true),
            intensity: Some(engine_term::Intensity::Normal),
            font: italic,
            ..Default::default()
        });
    ...
    pub fn apply_cmd_defaults(
        ...
        // and "$APPDIR" set in the Chatminal process. These will be
        // propagated to the child processes. Since some apps (including
        // Chatminal) use these variables to detect if they are running in
        ...
        cmd.env("TERM_PROGRAM", "Chatminal");
        cmd.env("TERM_PROGRAM_VERSION", crate::engine_version());
    }
}
...
pub(crate) fn compute_cache_dir() -> anyhow::Result<PathBuf> {
    ...
    Ok(crate::HOME_DIR.join(".local/share/chatminal"))
}
...
fn ensure_private_dir(path: &Path) -> anyhow::Result<()> {
    match private_dir_builder().create(path) {
        ...
        Err(err) => anyhow::bail!("create runtime dir {} failed: {err}", path.display()),
    }
}
...
/// Behavior when the program spawned by Chatminal terminates
#[derive(Debug, FromDynamic, ToDynamic, Clone, Copy, PartialEq, Eq, Default)]
...
impl FromDynamic for BoldBrightening {
    fn from_dynamic(
        value: &engine_dynamic::Value,
        options: engine_dynamic::FromDynamicOptions,
        ...
        match String::from_dynamic(value, options) {
            Ok(s) => match s.as_str() {
                ...
                s => Err(engine_dynamic::Error::Message(format!(
...
pub enum ImePreeditRendering {
    /// IME preedit is rendered by Chatminal itself
    #[default]
    Builtin,
