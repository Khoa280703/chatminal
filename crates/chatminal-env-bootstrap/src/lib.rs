...
pub fn set_chatminal_executable() {
    if let Ok(exe) = std::env::current_exe() {
        if let Some(dir) = exe.parent() {
            std::env::set_var("CHATMINAL_EXECUTABLE_DIR", dir);
        }
        std::env::set_var("CHATMINAL_EXECUTABLE", exe);
    }
}
...
pub fn fixup_appimage() {
    if let Some(appimage) = std::env::var_os("APPIMAGE") {
        ...
        // `Chatminal.AppImage foo`, which is super confusing for everyone!
        ...
        // CHATMINAL_EXECUTABLE_DIR is set by `set_chatminal_executable`
        // which is called before `fixup_appimage`
        if let Some(dir) = std::env::var_os("CHATMINAL_EXECUTABLE_DIR") {
        ...
        /// the correct environment when it is launched via Chatminal.
        /// However, if we are using the system terminal to spawn a portable
        /// AppImage then we want these to not take effect.
        fn clean_chatminal_config_env() {
...
pub fn bootstrap() {
    config::assign_version_info(
        ...
        engine_version::engine_target_triple(),
    );
    setup_logger();
    register_panic_hook();

    set_chatminal_executable();
    ...
    // Chatminal never sets WINDOWID and we don't want to inherit it from a
    // parent process.
    std::env::remove_var("WINDOWID");
    // Avoid vte shell integration kicking in if someone started
    // Chatminal or the mux server from inside gnome terminal.
    // upstream issue #2237
