...
VENDOR_DIR="${CHATMINAL_VENDOR_DEPS_DIR:-$ROOT_DIR/vendor/terminal-deps}"
QUIET=0
...
ensure_checkout() {
  ...
  if [[ -f "$sentinel_path" ]]; then
    log "chatminal terminal vendor dep ready: $name"
    return 0
  fi

  if [[ -d "$target_dir" ]] && find "$target_dir" -mindepth 1 -maxdepth 1 | read -r _; then
    echo "chatminal terminal vendor dep incomplete: $target_dir (missing $sentinel)" >&2
    exit 1
  fi

  log "hydrating chatminal terminal vendor dep: $name"
  rm -rf "$target_dir"
  mkdir -p "$(dirname "$target_dir")"

  local temp_dir
  temp_dir="$(mktemp -d "${TMPDIR:-/tmp}/chatminal-terminal-dep-XXXXXX")"
  git clone --depth 1 "$url" "$temp_dir" >/dev/null 2>&1
  rm -rf "$temp_dir/.git"
  mv "$temp_dir" "$target_dir"

  if [[ ! -f "$sentinel_path" ]]; then
    echo "chatminal terminal vendor dep hydrate failed: $name (missing $sentinel)" >&2
    exit 1
  fi
}
